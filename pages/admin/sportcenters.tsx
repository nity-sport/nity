...
const SportCentersAdminPage: React.FC = () => {
  const { user, canManageSportCenters, isSuperuser, isLoading: authLoading } = useAuth();
  const [sportCenters, setSportCenters] = useState<SportCenter[]>([]);
  const [loading, setLoading] = useState(true);
  const [pagination, setPagination] = useState<SportCentersResponse['pagination'] | null>(null);
      ...
      {loading ? (
        ...
        <>
          <div className={styles.sportCentersGrid}>
            {sportCenters && Array.isArray(sportCenters) && sportCenters.map(sportCenter => (
              <div key={sportCenter._id} className={styles.sportCenterCard}>
                ...
                <div className={styles.sportCenterContent}>
                  <h3 className={styles.sportCenterTitle}>{sportCenter.name}</h3>
                  ...
                  <div className={styles.sportCenterDetails}>
                    ...
                    {sportCenter.dormitory && (
                      <div className={styles.sportCenterDormitory}>
                        🏠 Dormitório: {formatPrice(sportCenter.dormitoryCosts)}
                      </div>
                    )}

                    {sportCenter.experienceCost && (
                      <div className={styles.sportCenterExperience}>
                        💫 Experiência: {formatPrice(sportCenter.experienceCost)}
                      </div>
                    )}
                  </div>
                  ...
                  <div className={styles.sportCenterMeta}>
                    ...
                    <div className={styles.ownershipInfo}>
                      {sportCenter.owner === user?.id ? (
                        ...
                        <span className={styles.otherOwnerSportCenter}>Outro proprietário</span>
                      ) : null}
                    </div>
                  </div>
                  ...
                  <div className={styles.sportCenterActions}>
                    {canEditSportCenter(sportCenter) ? (
                      <>
                        ...
                        <button
                          onClick={() => alert('Funcionalidade de duplicação será implementada')}
                          className={styles.duplicateButton}
                        >
                          Duplicar
                        </button>
                        <button
                          onClick={() => handleDeleteSportCenter(sportCenter._id)}
          ...
          {(!sportCenters || sportCenters.length === 0) && !loading && (
            <div className={styles.errorMessage}>
              ...
              <p>Crie seu primeiro sportcenter clicando no botão "Criar SportCenter".</p>
            </div>
          )}
