...
const handler = async (req: AuthenticatedRequest, res: NextApiResponse) => {
  try {
    ...
    if (!memberId || typeof memberId !== 'string' || !mongoose.isValidObjectId(memberId)) {
...
const handleRemoveMember = async (req: AuthenticatedRequest, res: NextApiResponse, teamId: string, memberId: string) => {
  try {
    ...
    if (team.scoutId.toString() !== scoutId) {
      return res.status(403).json({ message: 'Only the scout can remove members from this team' });
    }
