...
const handleAddMember = async (req: AuthenticatedRequest, res: NextApiResponse, teamId: string) => {
  try {
    ...
    if (team.scoutId.toString() !== scoutId) {
      return res.status(403).json({ message: 'Only the scout can add members to this team' });
    }
