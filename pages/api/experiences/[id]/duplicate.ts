...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (originalExperience.visibility !== 'public' && 
    ...
    if (!loggedInUser) {
      return res.status(404).json({ success: false, error: 'Usuário logado não encontrado.' });
    }
    ...
    const duplicateData = {
      ...
      availableDates: newDates ? newDates.map((date: string) => new Date(date)) : [],
      owner: {
