  ...
  switch (req.method) {
    ...
    case "PUT":
      try {
        ...
        if (user.role !== 'SUPERUSER') {
          return res.status(403).json({ error: "Apenas superusuários podem atualizar coaches" });
        }
    ...
    case "DELETE":
      try {
        ...
        if (user.role !== 'SUPERUSER') {
          return res.status(403).json({ error: "Apenas superusuários podem deletar coaches" });
        }
