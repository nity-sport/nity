...
const handleCreateCoupon = async (
  ...
  try {
    const { code, discountValue, discountPercentage, maxUses, expiresAt } =
      req.body;
    ...
    if (discountValue && discountPercentage) {
      return res.status(400).json({
        message: 'Provide either discountValue or discountPercentage, not both',
      });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'discountValue must be a positive number' });
    }
    ...
    if (
      ...
      return res.status(400).json({
        message: 'discountPercentage must be a number between 1 and 100',
      });
    }
    ...
    if (!user || user.role !== UserRole.SCOUT) {
      return res
        .status(403)
        .json({ message: 'Only Scout users can create coupons' });
    }
