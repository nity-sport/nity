    ...
    case "POST":
      ...
      try {
        ...
        if (!user || user.role !== 'SUPERUSER') {
          return res.status(403).json({ message: "Permissão negada. Apenas superusuários podem gerenciar facilities." });
        }
        ...
        if (existingFacility) {
          return res.status(400).json({ error: "Facility com este nome já existe" });
        }
